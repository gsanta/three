...
export type LightningStormParams = {
  size: number;
  minHeight: number;
  maxHeight: number;
  maxSlope: number;
  ...
  lightningParameters: object; // If you know the shape, you can make this more specific
  lightningMaterial: THREE.Material;
  onRayPosition?: (source: THREE.Vector3, dest: THREE.Vector3) => void;
  onLightningDown?: (lightningStrike: any) => void; // If you know the type of lightningStrike, replace `any`
};

class LightningStorm extends Object3D {
  constructor(stormParams: LightningStormParams) {
    super();
    // Parameters
    ...
    if (stormParams.onRayPosition !== undefined) {
      ...
      this.onRayPosition = function (source, dest) {
        ...
        const height = THREE.Math.lerp(stormParams.minHeight, stormParams.maxHeight, Math.random());
  ...
  update(time) {
    ...
    if (time >= this.nextLightningTime) {
      ...
      if (lightningMesh) {
        const lightningParams1 = THREE.LightningStrike.copyParameters(
        ...
        lightningParams1.deathTime =
          time +
          THREE.Math.lerp(this.stormParams.lightningMinDuration, this.stormParams.lightningMaxDuration, Math.random());
  ...
  clone() {
    return new LightningStorm(this.stormParams).copy(this);
  }

  stormParams: LightningStormParams;
}
