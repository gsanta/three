    ...
    SECTION("removes the tiles at the given pointer position")
    {
        DocumentStore documentStore =
            DocumentStoreBuilder()
                .withDrawing(DrawingBuilder().withBounds(Bounds::createWithPositions(0.0f, 0.0f, 2.0f, 2.0f)))
                .withDrawing(DrawingBuilder().withBounds(Bounds::createWithPositions(0.0f, 0.0f, 5.0f, 5.0f)))
                .build();
